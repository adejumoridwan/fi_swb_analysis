...
summarize_group <- function(data, group_col) {
  data %>%
    group_by({{ group_col }}) %>%
    summarize(
      mean_continuous_var1 = mean(SWB, na.rm = TRUE),
      sd_continuous_var1 = sd(SWB, na.rm = TRUE),
      mean_continuous_var2 = mean(FI, na.rm = TRUE),
      sd_continuous_var2 = sd(FI, na.rm = TRUE)
    )
}
...
```{r}
summarize_group(mics, welevel)
summarize_group(mics, WAGE)
summarize_group(mics, WM14)
summarize_group(mics, WB18)
summarize_group(mics, WM14)
summarize_group(mics, MSTATUS)
summarize_group(mics, WAGE)
